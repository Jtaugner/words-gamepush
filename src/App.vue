	...
	<div id="app"
		 :class="[
		  ...
		  showGameLocation ? 'locationOpened' : '',
		   ...
		   bannerShow ? 'app__bannerShowed': ''
		   ]"
	>
		<!--    <div class="close-level-help prev-location next-location"></div>-->
		<div class="levels"
			 @updateAll="updateAll()"
			 @bannerShowFalse="bannerShowFalse()"
			 @bannerShowTrue="bannerShowTrue()"
			...
			<div class="leaderBoardInfo">
				<template v-if="leaderBoard">
					<div
						...
						:class="[
							player.position < 21 ? 'leaderBoardInfo_' + player.position : '',
							 playerInfo && playerInfo.position === player.position ? 'leaderBoardInfo_mine' : '',
							 player.position === 20 ? 'lastRang' : ''
							 ]"
					>
							<div class="leaderBoardInfo__playerInfo">
								<div class="leaderBoardInfo__firstBlock">
									<div class="leaderBoardInfo__image"
										 :style="{background: 'url('+ player.avatar + ') center center no-repeat'}"
										 :class="[player.avatar ? '' : 'leaderBoardInfo__image_no']">
									</div>
									<div class="leaderBoardInfo__nameAndScore">
										<div class="leaderBoardInfo__name">{{ player.name ? player.name : 'Игрок #' + player.id }}</div>
										<div class="leaderBoardInfo__score">Звёзд: {{ player.score }}</div>
									</div>
								</div>
								<div
									class="leaderBoardInfo__rank"
									:class="player.position > 20 ? 'leaderBoardInfo_big' : ''"
								>
									<div class="leaderBoardInfo__rankInner"
										 :class="player.position > 99999 ? 'leaderBoardInfo__rankInner_bigRank' : ''"
									>
										{{player.position}}
									</div>
								</div>
...
<!--					</div>-->
<!--					<div class="shop__cart__buy-button" >-->
<!--						&lt;!&ndash;													<div class="shop__lastPrice">49</div>&ndash;&gt;-->
<!--						{{getItemPrice(4)}}-->
<!--					</div>-->
<!--				</div>-->
...
<!--					</li>-->
		...
		<div class="rules rules__notification" v-if="showLastLevelInfo && !notRussian">
			...
			<template v-else-if="wasUpdate">
				Предлагаем вам оценить наш зелёный фон и цветовое оформление!
...
const itemsPrices = [3, 13, 21, 42, 499];
const itemsOKPrices = [20, 89, 149, 299, 499];
let purchaseItem;
let buyItem;
...
console.log('www: ', window.innerWidth, window.innerHeight)
...
// 	gp.ads.on('sticky:close', () => {console.log('close')});
	...
	GP.payments.on('purchase', ({ product, purchase }) => {
		...
		if(buyItem === 2) TIPS = allTips[0];
		if(buyItem === 3) TIPS = allTips[1];
		if(buyItem === 4) TIPS = allTips[2];
		if(buyItem === 5) TIPS = allTips[3];
		let it = 'newBuy-' + buyItem;
		params({[it]: 1});
		document.querySelector('.levels').dispatchEvent(new CustomEvent("buyTips"));
		setStats();
		GP.payments.consume({ tag: purchaseItem });
	});
...
function initPlayer(gp) {
	...
	gp.player.on('fetchFields', (success) => {
		...
		if(success){
			...
			playerGame = gp.player;
			if(gp.player.has('allDoneWords')){
				console.log('GP have allDoneWords')
				isRules = false;
				let newData = JSON.parse(gp.player.get('allDoneWords'));
			...
			if(gp.player.has('locationDoneWords')){
				let locationData = JSON.parse(gp.player.get('locationDoneWords'));
				let serverWords = fixDoneWords(decompressLocationWords(locationData), true);
			...
			let dataTips = gp.player.get('tips');
...
export default {
	...
	computed:{
		...
		linkToGroup(){
			if(this.platformType === 'OK') return 'https://ok.ru/group/70000002237520';
	...
	methods:{
		...
		getItemPrice(item){
			if(this.platformType === 'OK') return itemsOKPrices[item] + ' ОК';
			let gol = 'голоса';
			if(item === 1) gol = 'голосов';
			if(item === 2) gol = 'голос';
			return itemsPrices[item] + ' ' + gol;
		},
			...
			if(this.nowWords.includes(this.wordFromLetter) && !this.doneWords.includes(this.wordFromLetter)){
				...
				setTimeout(()=>{
					try{
						...
						scrollEl.scrollIntoView({behavior: 'auto', block: "nearest", inline: "center"});
					}catch(e){
